#!/usr/bin/env python3
"""
ChatGPT Plugin Registration URLs Extractor - WINDOWS FIXED VERSION
Process all 4 batches and extract OAUTH registration URLs
Run this script on your local machine with: python process_all_batches_locally_WINDOWS_FIXED.py
"""
...
BATCH_DIR = r"C:\Users\Puranjay Kaushik\Downloads\chatgpt-plugins-data-main\chatgpt-plugins-data-main"
BATCH_FILES = [
    os.path.join(BATCH_DIR, "batch_1_data.json"),
    os.path.join(BATCH_DIR, "batch_2_data.json"),
    os.path.join(BATCH_DIR, "batch_3_data.json"),
    os.path.join(BATCH_DIR, "batch_4_data.json")
]
...
def make_request(batch_num, app_ids):
    ...
    try:
        # FIX: Use encoding='utf-8' to handle Unicode properly on Windows
        result = subprocess.run(curl_cmd, capture_output=True, text=True, timeout=60, encoding='utf-8', errors='replace')
...
def main():
    ...
    output_dir = os.path.dirname(BATCH_FILES[0])
    json_output = os.path.join(output_dir, "all_registration_urls_final.json")
    csv_output = os.path.join(output_dir, "all_registration_urls_final.csv")
